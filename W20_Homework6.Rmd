...
dim(samples_df)
n_distinct(samples_df$participant_id)
```

### Diet data
Import the dietary information from study participants, join with sample data imported in section above. Note that after the join we have the same number of participants in the resulting data frame. The result contains the diet & sample data. 
```{r}
...
* average dietary fiber in males and females
...
Is there a correlation between the abundance of Ruminococcus bromii (OTU_XXX) and fiber in the diet? 
```{r}
# data formatting 
...
Does Ruminococcus bromii abundance significantly differ between any race/ethnicity?
```{r}
# data formatting 
```
...
Is there a correlation between the abundance of Bifidobacterium faecale (OTU_XXX) and fiber in the diet? 
```{r}
# data formatting 
```
